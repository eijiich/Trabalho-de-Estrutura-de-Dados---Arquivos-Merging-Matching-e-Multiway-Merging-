...
void randomTrait(char *pkmnTrait);
...
void randomTrait(char *pkmnTrait){
  //Variáveis para sortear uma posição na lista de tipos e naturezas do pokemon
  int randNaturePos, randTypePos1, randTypePos2;
  randNaturePos = rand() % 25;
  randTypePos1 = rand() % 18;
  randTypePos2 = rand() % 18;

  //Existem pokemon mono e dual-type, haverá 50% de chance de ser um dos dois
  if(rand() % 2){
    sprintf(pkmnTrait, "%s %s TYPE", pokemonNatureList[randNaturePos], pokemonTypeList[randTypePos1]);
  }
  else{
    if(randTypePos1 != randTypePos2){
      while (randTypePos1 == randTypePos2) {
        randTypePos2 = rand() %18;
      }
    }
    sprintf(pkmnTrait, "%s %s/%s TYPE", pokemonNatureList[(rand() % 25)], pokemonTypeList[randTypePos1], pokemonTypeList[randTypePos2]);
  }
}
...
void generateFile(int n, char* nome){
  ...
  for(i=0; i<n; i++){
    ...
    randomName = (rand() % 807);
    for(j=0; j<20; j++){
      pkmn.pkmnName[j] = pkmnList[(randomName)][j];
    }
    randomTrait(pkmn.pkmnTrait);
